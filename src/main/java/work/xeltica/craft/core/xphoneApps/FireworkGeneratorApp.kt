...
class FireworkGeneratorApp : AppBase() {
    ...
    private fun openFireworkLaunchApp(player: Player) {
        val list = FireworkType.values()
            .map { f ->
                MenuItem(
                    f.typeName,
                    { chooseFireworkColor(player, f.type) },
                    f.material
                )
            }
            .toList()
    ...
    private fun chooseFireworkColor2(player: Player, type: FireworkEffect.Type, color: Color) {
        ...
        list2.add(0,
            MenuItem(
                "なし",
                { chooseFireworkPower(player, type, color, null) },
                Material.BARRIER
            )
        )
    ...
    private fun chooseFireworkAttributes(
        ...
        ui.openMenu(
            player, "花火の属性を選んでください", listOf(
                MenuItem(
                    "点滅エフェクト (現在: " + (if (attr.flicker) "オン" else "オフ") + ")",
                    {
                ...
                MenuItem(
                    "軌跡エフェクト (現在: " + (if (attr.trail) "オン" else "オフ") + ")",
                    {
                ...
                MenuItem(
                    "ダウンロード",
                    { downloadFirework(player, type, color, color2, power, attr) },
