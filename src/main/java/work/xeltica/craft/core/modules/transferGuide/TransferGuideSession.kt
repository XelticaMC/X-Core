 ...
 * メソッド名規則
 *  openXxx: Xxxの為のメニューを開く
 *  showXxx: チャット欄にXxxを表示する(アプリは閉じる)
 *  verifyXxx: デバッグ用の路線データチェック操作
 * @author Knit prg.
 */
class TransferGuideSession(val player: Player) {
     ...
     * 駅を設定して対応するメニューに戻ります。
     */
    private fun setStationAndReturn(newId: String, stationChoiceTarget: StationChoiceTarget) {
    ...
    private fun openMain() {
        ...
        val items = arrayListOf(
            MenuItem("出発地点:$startStationName", { openChooseStation(StationChoiceTarget.START) }, Material.LIME_BANNER),
            MenuItem("到着地点:$endStationName", { openChooseStation(StationChoiceTarget.END) }, Material.RED_BANNER),
            MenuItem("出発地点と到着地点を入れ替える", { reverseChosenStations() }, Material.LEVER),
            MenuItem("計算開始", { showRoute() }, Material.COMMAND_BLOCK_MINECART),
            MenuItem("駅情報", { openStationInfo() }, Material.CHEST_MINECART),
        )
        if (data.availableWorlds[player.world.name] == "main") {
            items.add(MenuItem("お気に入りの駅", { openFavorites() }, Material.OAK_SIGN))
        }
        items.add(MenuItem("乗換案内について&データ更新内容", { showAbout() }, Material.ENCHANTED_BOOK))
        ...
        if (player.isOp || player.uniqueId.toString() == knit) {
            ...
            items.addAll(
                arrayListOf(
                    MenuItem("デバッグメニュー", { openDebug() }, head ?: ItemStack(Material.NETHERITE_AXE), isShiny = true)
                )
            )
        }
     ...
     */
    private fun openChooseStation(stationChoiceTarget: StationChoiceTarget) {
        when (data.availableWorlds[player.world.name]) {
            "main" -> openChooseStationMain(stationChoiceTarget)
            "wild" -> openChooseStationWild(stationChoiceTarget)
     ...
     */
    private fun openChooseStationMain(stationChoiceTarget: StationChoiceTarget) {
        val items = arrayListOf(
            MenuItem("近い順", { openChooseStationNear(stationChoiceTarget) }, Material.DIAMOND_HORSE_ARMOR),
            MenuItem("五十音順", { openChooseStationAiueo(stationChoiceTarget) }, Material.BOOK),
            MenuItem("会社・路線別", { openChooseStationLine(stationChoiceTarget) }, Material.SPRUCE_DOOR),
            MenuItem("最寄自治体別", { openChooseStationMuni(stationChoiceTarget) }, Material.FILLED_MAP),
        ...
        if (stationChoiceTarget != StationChoiceTarget.FAVORITES_ADD) {
            items.add(MenuItem("お気に入りの駅", { openChooseStationFavorites(stationChoiceTarget) }, Material.OAK_SIGN))
        }
        val goToMenu: Consumer<MenuItem> = when (stationChoiceTarget) {
            StationChoiceTarget.START, StationChoiceTarget.END -> Consumer { openMain() }
            StationChoiceTarget.INFO -> Consumer { openStationInfo() }
            StationChoiceTarget.FAVORITES_ADD -> Consumer { openFavorites() }
        }
     ...
     * GUI: 駅選択/五十音順
     */
    private fun openChooseStationAiueo(stationChoiceTarget: StationChoiceTarget) {
        val items = ArrayList<MenuItem>()
        JapaneseColumns.values().forEach { column ->
            items.add(
                MenuItem("${column.firstChar}行", { openChooseStationAiueo(stationChoiceTarget, column) }, Material.BOOK)
            )
        }
        items.add(MenuItem("戻る", { openChooseStation(stationChoiceTarget) }, Material.REDSTONE_TORCH))
     ...
     * GUI: 駅選択/五十音順/○行
     */
    private fun openChooseStationAiueo(stationChoiceTarget: StationChoiceTarget, column: JapaneseColumns) {
        val items = ArrayList<MenuItem>()
        column.chars.forEach { char ->
            items.add(
                MenuItem(char, { openChooseStationAiueo(stationChoiceTarget, column, char) }, Material.BOOK)
            )
        }
        items.add(MenuItem("戻る", { openChooseStationAiueo(stationChoiceTarget) }, Material.REDSTONE_TORCH))
    ...
    private fun openChooseStationAiueo(stationChoiceTarget: StationChoiceTarget, column: JapaneseColumns, char: String) {
        ...
        stations.value.forEach { station ->
            items.add(
                MenuItem(station.nameAndNumber, { setStationAndReturn(station.id, stationChoiceTarget) }, Material.MINECART)
            )
        }
        items.add(MenuItem("戻る", { openChooseStationAiueo(stationChoiceTarget, column) }, Material.REDSTONE_TORCH))
     ...
     * GUI: 駅選択/会社選択
     */
    private fun openChooseStationLine(stationChoiceTarget: StationChoiceTarget) {
        val items = ArrayList<MenuItem>()
        data.companies.forEach { company ->
            items.add(MenuItem(company.value.name ?: company.key, { openChooseStationLine(stationChoiceTarget, company.value) }, Material.SPRUCE_DOOR))
        }
        items.add(MenuItem("戻る", { openChooseStation(stationChoiceTarget) }, Material.REDSTONE_TORCH))
    ...
    private fun openChooseStationLine(stationChoiceTarget: StationChoiceTarget, company: KCompany) {
        ...
        lines.value.forEach { line ->
            items.add(
                MenuItem(line.name ?: line.id, { openChooseStationLine(stationChoiceTarget, company, line) }, Material.RAIL)
            )
        }
        items.add(MenuItem("戻る", { openChooseStationLine(stationChoiceTarget) }, Material.REDSTONE_TORCH))
    ...
    private fun openChooseStationLine(stationChoiceTarget: StationChoiceTarget, company: KCompany, line: KLine) {
        ...
        items.add(MenuItem("戻る", { openChooseStationLine(stationChoiceTarget, company) }, Material.REDSTONE_TORCH))
    ...
    private fun openChooseStationMuni(stationChoiceTarget: StationChoiceTarget) {
        ...
        munis.value.forEach { muni ->
            items.add(
                MenuItem(muni.name ?: muni.id, { openChooseStationMuni(stationChoiceTarget, muni) }, Material.FILLED_MAP)
            )
        }
        items.add(MenuItem("戻る", { openChooseStation(stationChoiceTarget) }, Material.REDSTONE_TORCH))
    ...
    private fun openChooseStationMuni(stationChoiceTarget: StationChoiceTarget, muni: KMuni) {
        ...
        items.add(MenuItem("戻る", { openChooseStationMuni(stationChoiceTarget) }, Material.REDSTONE_TORCH))
    ...
    private fun openChooseStationNear(stationChoiceTarget: StationChoiceTarget) {
        ...
        stations.value.forEach { station ->
            ...
            items.add(
                MenuItem("${station.nameAndNumber}(約${distance})", { setStationAndReturn(station.id, stationChoiceTarget) }, Material.MINECART)
            )
        }
        items.add(MenuItem("戻る", { openChooseStation(stationChoiceTarget) }, Material.REDSTONE_TORCH))
     ...
     * GUI: 駅選択/お気に入りの駅
     */
    private fun openChooseStationFavorites(stationChoiceTarget: StationChoiceTarget) {
        val items = ArrayList<MenuItem>()
        favorites.forEach { stationId ->
            val station = data.stations[stationId] ?: return@forEach
            items.add(MenuItem(station.name ?: stationId, { setStationAndReturn(stationId, stationChoiceTarget) }, Material.MINECART))
        }
        items.add(MenuItem("戻る", { openChooseStation(stationChoiceTarget) }, Material.REDSTONE_TORCH))
    ...
    private fun calcRoute(startId: String?, endId: String?, reverse: Boolean = false): KRoute? {
        ...
        fun loopADebugString(): String {
            return "step=A${i}\n" +
                    ...
                    "opened=${opened.toList().joinToString { "${it.first.id}(${it.second.elapsedTime}:${it.second.expectedTime})" }}\n" +
        ...
        knitA@ while (i < data.loopMax) {
            ...
            opened.forEach {
                ...
                val estimatedTime = (xDistance + yDistance) / 8
                opened[it.key] = OpenedValue(it.value.elapsedTime, it.value.elapsedTime + estimatedTime)
            }
            if (data.consoleDebug) {
                logger.info("[TransferGuide(debug)] ${loopADebugString()}")
            }

            //探索候補の駅の内、最も時間が短いものを探す
            val minOpenedStation = opened.minByOrNull { it.value.expectedTime }

            //見つからない場合、両駅間の経路は存在しないものと考えられる
        ...
        knitB@ while (j < data.loopMax) {
            ...
            if (candidates.isEmpty()) {
                ...
                while (k < data.loopMax) {
                    ...
                    if (routeArrayList.isEmpty()) {
                        gui.error(player, "駅探索Baに失敗しました。")
                        logger.warning("[TransferGuideData] 駅探索失敗Ba\n${loopBDebugString()}")
                        return null
                    }
                    if (data.consoleDebug) {
            ...
            if (max == null) {
                gui.error(player, "駅探索Bに失敗しました。")
                logger.warning("[TransferGuideData] 駅探索失敗B\n${loopBDebugString()}")
                return null
            }
    ...
    private fun openStationInfo() {
        ...
        items.add(MenuItem("駅選択", { openChooseStation(StationChoiceTarget.INFO) }, Material.COMPASS))
        items.add(MenuItem("戻る", { openMain() }, Material.REDSTONE_TORCH))
    ...
    private fun openFavorites() {
        val items = arrayListOf(
            MenuItem("追加", { openChooseStation(StationChoiceTarget.FAVORITES_ADD) }, Material.BLACK_DYE),
            MenuItem("削除", { openFavoritesDelete() }, Material.WHITE_DYE),
            MenuItem("順番変更", { openFavoritesSort(null) }, Material.CHEST),
            MenuItem("戻る", { openMain() }, Material.REDSTONE_TORCH),
        )
        gui.openMenu(player, "お気に入りの駅", items)
    }
     ...
     * GUI: お気に入りの駅/削除
     */
    private fun openFavoritesDelete() {
        val items = ArrayList<MenuItem>()
        favorites.forEach { station ->
            items.add(MenuItem(data.stations[station]?.name ?: station, { deleteFavoritesAndReturn(station) }, Material.MINECART))
        }
        items.add(MenuItem("戻る", { openFavorites() }, Material.REDSTONE_TORCH))
     ...
     */
    private fun deleteFavoritesAndReturn(station: String) {
     ...
     * GUI: お気に入りの駅/並び替え
     */
    private fun openFavoritesSort(first: String?) {
        val items = ArrayList<MenuItem>()
        ...
        favorites.forEach { station ->
            val nextAction: Consumer<MenuItem> = when (first) {
                null -> {
                    Consumer { openFavoritesSort(station) }
                }

                station -> {
                    Consumer { openFavoritesSort(null) }
                }

                else -> {
                    Consumer { sortFavoritesAndReturn(first, station) }
                }
            }
     ...
     */
    private fun sortFavoritesAndReturn(first: String, second: String) {
    ...
    private fun openDebug() {
        val items = arrayListOf(
            ...
            MenuItem("全経路チェック", { showVerifyAllRoutesWarn(false) }, Material.SPYGLASS),
            MenuItem("全経路チェック(到達可能性のみ)", { showVerifyAllRoutesWarn(true) }, Material.SPYGLASS),
            MenuItem("戻る", { openMain() }, Material.REDSTONE_TORCH),
        )
     ...
     */
    private fun showVerifyAllRoutesWarn(onlyReachable: Boolean) {
        gui.openTextInput(
